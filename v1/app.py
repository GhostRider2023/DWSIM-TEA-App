...
# Support direct invocation with `python app.py` by relaunching via Streamlit.
...
def resolve_raw_material_db_path() -> Path | None:
    ...
    candidate_names = [
        "dwsim_raw_material_costs.csv",
        "raw_material_costs.csv",
        "material_prices.csv",
        "pricing_database.csv",
    ]
    ...
    for directory in candidate_dirs:
        for path in directory.rglob("*.csv"):
            ...
            if "raw" in name and ("material" in name or "price" in name):
                return path
            if "material" in name and "price" in name:
                return path
    return None


def load_raw_material_db() -> pd.DataFrame:
    path = resolve_raw_material_db_path()
    if path is not None:
        try:
